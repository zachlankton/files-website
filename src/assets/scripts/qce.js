(() => {
	...
	ShadowRoot.prototype.qsa = ShadowRoot.prototype.querySelectorAll

	void function setupCustomElements() {
	...
	function setupCustomElement(template) {
		...
		class newElm extends HTMLElement {
			...
			constructor() {
				...
				if (scriptConnected) this.qce_connected = prepareScript(scriptConnected, "elms")
			...
			connectedCallback() {
				const ts = this.shadowRoot;
				const elms = {}
				const idElements = ts.querySelectorAll("[id]");
				[...idElements].map((e) => elms[e.id] = e)
				this.qce_connected && this.qce_connected(elms);
			}
		...
		function prepareScript(script, args = "") {
			...
			return eval(`[async function ${_name}(${args}){ ${newScript} }][0]`);
		}
	}
